...
def Object_tracking(Yolo, video_path, output_path, input_size=416, show=False, CLASSES=YOLO_COCO_CLASSES, score_threshold=0.3, iou_threshold=0.45, rectangle_colors='', Track_only = []):
    try:
        ...
        ########### End of code for live video feed ########################
            ...
            # saving image to video (Comment out the 5 lines below to not write to a video)
            for ids in out_video_dict:
                to_write = out_video_dict[ids]
                out = cv2.VideoWriter(output_path + str(ids) + ".mp4", codec, fps, (width, height)) # output_path must be .mp4
                #print(type(to_write[0]))
                for images_to_write in to_write:
                    opencvImage = cv2.cvtColor(np.array(images_to_write), cv2.COLOR_RGB2BGR)
...
model.eval()
Object_tracking(yolo, video_path, "detection", input_size=YOLO_INPUT_SIZE, show=True, iou_threshold=0.1, rectangle_colors=(255,0,0), 
